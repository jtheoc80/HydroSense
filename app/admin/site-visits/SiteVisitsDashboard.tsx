...
export default function SiteVisitsDashboard({ visits }: { visits: DashboardVisit[] }) {
  ...
  const filtered = useMemo(() => visits.filter((visit) => {
    ...
    if (date && visit.scheduled_start.slice(0, 10) !== date) return false;
    return true;
  }), [visits, search, status, readiness, rep, date]);

  const queues = [
    ["Today", (v: DashboardVisit) => new Date(v.scheduled_start).toDateString() === new Date().toDateString()],
  ...
  const upcoming = filtered.filter((visit) => {
    const when = new Date(visit.scheduled_start).getTime();
    return when >= now - day && when <= now + 60 * day;
  });
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8" aria-label="Operational queues">
          {queues.map(([label, predicate]) => {
            ...
            return <div key={label} className="rounded-xl border border-white/10 bg-ink-900 p-4"><div className="font-mono text-2xl text-fog-50">{count}</div><div className="mt-1 text-xs leading-snug text-fog-300">{label}</div></div>;
          })}
        </section>
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-ink-900">
          <div className="border-b border-white/10 px-5 py-4"><h2 className="font-semibold text-fog-50">Upcoming and recent</h2><p className="text-xs text-fog-400">{upcoming.length} matching visits</p></div>
